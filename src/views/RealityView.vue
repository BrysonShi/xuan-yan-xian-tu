...
const highlightWords = ['灵脉', '陈墨'];

const currentOptions = ref([
  { id: 'A', text: '与陈墨同去后山灵脉', description: '可能遇到奇遇，也可能遇到危险' },
  { id: 'B', text: '独自去练功场修炼', description: '稳扎稳打，提升修为' },
  { id: 'C', text: '回房启动命运模拟', description: '消耗10天机点，窥探命运' },
]);
...
function handleChoice(opt) {
  ...
  if (opt.id === 'C') {
    ...
    // 示例：添加后续剧情
    setTimeout(() => {
      storyText.value = opt.id === 'A'
        ? `你与陈墨一同走向后山。山路蜿蜒，越往深处灵气越浓。\n\n走了约莫半个时辰，你们在一处山涧旁发现了微弱的灵光……`
        : `你来到练功场，选了一处僻静角落盘膝坐下。\n\n运转功法，灵气缓缓汇入经脉。修为在缓慢但稳定地增长。`;
      showChoices.value = false;
      nextTick(() => { showChoices.value = false; });
    }, 400);
  }
}
...
onMounted(() => {
  // 由 onTextComplete 回调控制 showChoices，不再重复触发
});
</script>
